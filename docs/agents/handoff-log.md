...
## 2026-05-29 13:45 - Coordinator Agent

- 领取任务：落实 `robots_disallow` 禁抓规则，并从默认来源删除已知禁止来源。
- 完成内容：抓取遇到 `robots_disallow` 时自动把来源标记为 `blocked_by_policy`，记录禁止策略说明；从默认候选来源移除 `https://www.cbea.com/`，替换为中国电力企业联合会新闻中心候选源；启动种子会清理旧的电池中国网默认来源，并把历史 `robots_disallow` 来源标记为禁止策略状态。
- 验证结果：待运行后端测试、ruff、前端构建，并刷新本地开发库。
- 遗留问题：新增替代候选源仍需管理员手动测试 robots 和页面结构。
- 下一步建议：来源管理页中看到 `blocked_by_policy` 的来源不要再启用；默认候选来源继续保持 `manual_only` 逐个测试。
