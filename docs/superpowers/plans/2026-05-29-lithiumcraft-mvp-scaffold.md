...
**Goal:** 搭建一个面向内部投研的锂电池工艺情报工作台，支持公开只读首页、管理员后台、精选公开来源每日抓取、AI 摘要分类、情报检索、每日摘要和 Docker 部署。

**Architecture:** 项目采用单仓库前后端分离架构。Vue 3 前台公开展示首页、情报列表、情报详情和每日摘要，`/admin/*` 管理页通过 Bearer JWT 调用 FastAPI `/api/v1`；FastAPI 将公开只读 API 与管理员 API 拆分；Celery Beat 每天 `Asia/Shanghai 07:00` 触发抓取、`07:30` 触发每日摘要；PostgreSQL 存储业务数据，Redis 承担队列和缓存。
...
- `frontend-app`：Vue 3 公开前台与管理员工作台。
...
- [x] **Task 1: 规范文档落地**
  ...
  - 明确公开只读前台、精选 5-10 个来源、每日 7 点自动抓取、每日摘要、单管理员后台和管理 API 鉴权。

- [x] **Task 1.1: Agent 协作板落地**
...
- [x] **Task 2: 后端领域模型与 API**
  ...
  - 情报、每日摘要和分类接口公开只读；来源、设置、抓取任务、状态修改等管理接口使用 Bearer JWT。
...
- [x] **Task 4: 每日摘要任务**
  - Celery Beat 每天 07:00 抓取全部启用来源，07:30 生成 `DailyBrief`。
  - 每日摘要包含总览、重点条目和分类摘要。
  - 首次启动不创建演示情报；未经确认的来源保持 disabled，避免自动访问外部网站。

- [x] **Task 5: Vue 统一工作台**
  - 创建登录页、仪表盘、情报列表、情报详情、每日摘要、来源管理、抓取日志、系统设置。
  - `/`、`/intelligence`、`/daily-briefs` 公开访问；`/admin/*` 未登录自动跳转登录；API client 登录后附加 Bearer token。
  - 已安装前端依赖并通过 `npm run build`。

- [x] **Task 5.1: 公开首页与手动抓取调整**
  - `/` 改为公开首页，首页无真实数据时显示每日 7 点或管理员手动抓取的空状态。
  - 新增 `POST /api/v1/crawl-tasks/enabled`，管理端抓取日志页新增“抓取全部启用来源”按钮。
  - 公开列表和详情默认只展示 `active` 情报；`blocked`、`archived` 仅管理员可查看。
...
- 未登录可访问 `/`、`/intelligence`、`/intelligence/:id`、`/daily-briefs`。
- 单管理员可登录 `/admin/*` 管理入口。
- 来源管理可新增、禁用、查看精选来源。
- 抓取任务遵守来源状态、robots、限频和失败重试。
- 情报可检索、筛选、查看详情，状态支持 `active`、`blocked`、`archived`。
- 每日 07:00 自动抓取，07:30 生成摘要；管理员可手动触发单来源或全部启用来源抓取。
- 每日摘要可按日期查看。
